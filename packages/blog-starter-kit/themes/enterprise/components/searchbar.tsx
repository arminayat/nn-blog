...
export const Search = () => {
  ...
  const search = async (query: string) => {
    ...
    timerRef.current = setTimeout(async () => {
      ...
      const posts = data.searchPostsOfPublication.edges.map(
        (edge) => edge.node,
      );
  ...
  return (
    <div className="relative col-span-1">
      <input
        ...
        className="w-full rounded-full border border-slate-200 bg-white px-4 py-2 text-base focus:bg-white dark:border-neutral-800 dark:bg-neutral-800 dark:text-neutral-50 dark:placeholder:text-neutral-400 dark:hover:bg-neutral-950"
      />
      {query && (
        <>
          {isSearching && (
