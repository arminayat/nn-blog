...
type TableOfContentsItem =
  PostFullFragment["features"]["tableOfContents"]["items"][number];
...
const Toc = ({
  ...
  return (
    <ul className="mt-3 flex flex-col gap-3 pl-5 font-medium text-slate-800 dark:text-neutral-200">
      {children.map((item) => (
        <li key={item.id}>
          <a
            href={`#heading-${item.slug}`}
            className="hover:text-primary-600 hover:bg-primary-50 dark:hover:text-primary-500 underline dark:hover:bg-neutral-800 font-normal"
          >
            {item.title}
          </a>
...
export const PostTOC = () => {
  ...
  return (
    <div className="w-full px-5">
      <div className="mx-auto w-full max-w-screen-md rounded-lg border p-5 text-base leading-snug dark:border-neutral-800 dark:text-neutral-50 md:p-8 md:text-lg">
